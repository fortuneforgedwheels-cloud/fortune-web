...
function extractConfigs(templatePath) {
  const raw = readTemplateJson(templatePath);
  const section = Object.values(raw.sections || {}).find((s) => s.type === 'ff-fitment-guide');
  if (!section) return [];
  ...
  for (const block of Object.values(section.blocks || {})) {
    ...
    for (const cfg of ['a', 'b']) {
      const front = b[`${cfg}_front`];
      const rear = b[`${cfg}_rear`];
...
for (const { slug, file } of TEMPLATES) {
  ...
  fitments[slug] = extractConfigs(filePath);
