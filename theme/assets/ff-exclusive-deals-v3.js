  ...
  function init(root) {
    ...
    sheenBlocks(modal).forEach(function (sheen) {
      ...
      trigger.addEventListener('click', function (event) {
        event.preventDefault();
        var wrapEl = finishesRoot(modal);
        var current = wrapEl ? wrapEl.getAttribute('data-active-sheen') : '';
        if (current === family) clearActiveSheen(modal);
        else setActiveSheen(modal, family);
      });

      sheen.addEventListener('mouseenter', function () {
        if (hoverTimer) {
    ...
    if (wrap) {
      wrap.addEventListener('mouseleave', function () {
        hoverTimer = setTimeout(function () {
          var selected = modal.querySelector('[data-ff-xd-sheen].is-selected');
          // Keep open only while hovering; collapse when leaving the finish area
          clearActiveSheen(modal);
