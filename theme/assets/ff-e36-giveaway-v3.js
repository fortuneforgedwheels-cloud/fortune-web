...
(function () {
  ...
  function initQty(root) {
    ...
    function refresh() {
      ...
      var cta = qs(root, '[data-ff-e36-cta]');
      if (cta) {
        var prefix = wrap.getAttribute('data-cta-prefix') || 'ENTER NOW — ';
        cta.textContent = prefix + formatMoney(unit * qty, moneyFormat);
      }
    }
  ...
  function initRulesModal(root) {
    var modal = qs(root, '[data-ff-e36-rules-modal]');
    if (!modal) return;
    var sheet = qs(modal, '[data-ff-e36-rules-sheet]');
    var scrollEl = qs(modal, '[data-ff-e36-terms-scroll]');
    var lastFocus = null;
    ...
    function openModal(e) {
      ...
      modal.hidden = false;
      document.body.classList.add('ff-e36-rules-open');
      window.setTimeout(function () {
        var focusTarget = scrollEl || sheet;
    ...
    function closeModal(e) {
      ...
      modal.hidden = true;
      document.body.classList.remove('ff-e36-rules-open');
    ...
    qsa(root, '[data-ff-e36-open-rules]').forEach(function (btn) {
      btn.addEventListener('click', openModal);
    });
    qsa(root, '[data-ff-e36-close-rules]').forEach(function (btn) {
      btn.addEventListener('click', closeModal);
    });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && !modal.hidden) closeModal(e);
    });
  }
  ...
  function initRoot(root) {
    ...
    function syncStoryOverflow() {
      ...
      /* Collapsed by design — always offer VIEW FULL BUILD when body exists */
      storyPanel.classList.remove('is-short');
      if (storyMoreBtn) {
        storyMoreBtn.hidden = false;
      }
    }
