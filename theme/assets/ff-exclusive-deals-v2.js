(function () {
  function finishSelects(root) {
    return root.querySelectorAll('[data-ff-xd-finish-select]');
  }

  function selectedFinish(root) {
    var found = '';
    finishSelects(root).forEach(function (select) {
  ...
  function resetFinishSelects(root) {
    finishSelects(root).forEach(function (select) {
      select.value = '';
  ...
  function markFilledSelects(root) {
    finishSelects(root).forEach(function (select) {
      select.classList.toggle('is-filled', Boolean(String(select.value || '').trim()));
    });
  }
  ...
  function init(root) {
    ...
    finishSelects(modal).forEach(function (select) {
      select.addEventListener('change', function () {
        var chosen = String(select.value || '').trim();
        finishSelects(modal).forEach(function (other) {
