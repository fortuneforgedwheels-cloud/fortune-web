(function () {
  if (window.__ffShopByVehicleV3) return;
  window.__ffShopByVehicleV3 = true;
  ...
  function renderFitmentCard(container, chassis, configs, source, guideSlug, onPick) {
    ...
    var html = '<div class="ff-sbv__fitment-header">'
      ...
      + '<span class="ff-sbv__fitment-label" data-ff-sbv-fitment-label>Select a fitment</span>'
      + '</div>'
  ...
  function parseFitmentSize(spec) {
    var s = String(spec || '').trim();
    if (!s) return null;
    var m = s.match(/(\d+(?:\.\d+)?)\s*[xX×]\s*(\d+(?:\.\d+)?)/);
    if (m) {
      return { diameter: parseFloat(m[1]), width: parseFloat(m[2]), raw: s };
    }
    var d = s.match(/(\d+(?:\.\d+)?)\s*["″]?/);
    if (d) return { diameter: parseFloat(d[1]), width: null, raw: s };
    return null;
  }
  ...
  function variantMatchesFitment(variant, parsed) {
    if (!variant || !parsed || isNaN(parsed.diameter)) return false;
    var o1 = normalizeOptionValue(variant.option1 || variant.title);
    if (!o1) return false;

    // Monoblock-style Diameter option: "20" / 20"
    if (/^\d+(?:\.\d+)?$/.test(o1)) {
      return parseFloat(o1) === parsed.diameter;
    }

    // Beadlock/SIZE option: 18x11 / 18X11
    var parts = o1.match(/^(\d+(?:\.\d+)?)X(\d+(?:\.\d+)?)/);
    if (parts) {
      var d = parseFloat(parts[1]);
      var w = parseFloat(parts[2]);
      if (d !== parsed.diameter) return false;
      if (parsed.width == null || isNaN(parsed.width)) return true;
      return w === parsed.width;
    }

    return false;
  }

  function resolveVariantId(design, sizeSpec) {
    if (!design) return null;
    var variants = design.variants || [];
    var parsed = parseFitmentSize(sizeSpec);
    if (!parsed || !variants.length) return design.variantId || null;

    var i;
    for (i = 0; i < variants.length; i++) {
      if (variantMatchesFitment(variants[i], parsed) && variants[i].available !== false) {
        return variants[i].id;
      }
    }
    for (i = 0; i < variants.length; i++) {
      if (variantMatchesFitment(variants[i], parsed)) return variants[i].id;
    }
    return design.variantId || null;
  }
  ...
  function renderDesigns(container, products, onSelect) {
    ...
    container.querySelectorAll('.ff-sbv__design').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        btn.classList.add('is-selected');
        if (onSelect) {
          var idx = Number(btn.getAttribute('data-design-index'));
          onSelect(btn, products[idx] || null);
        }
      });
    });
    ...
    if (onSelect) {
      ...
      if (first) {
        var firstIdx = Number(first.getAttribute('data-design-index'));
        onSelect(first, products[firstIdx] || null);
      }
    }
  }


  function cleanLineProps(props) {
    var out = {};
    Object.keys(props || {}).forEach(function (key) {
      ...
      val = String(val).trim();
      if (!val) return;
      // Never surface guide vendor wording in cart/checkout
      if (/apex\s*guide|fortune\s*forged\s*guide|fitment\s*source/i.test(key + ' ' + val)) return;
      out[key] = val;
    });
    return out;
  }
  ...
  function buildCartItems(state) {
    ...
    var frontVariantId = resolveVariantId(design, fit.front);
    var rearVariantId = resolveVariantId(design, fit.rear);
    if (!frontVariantId && !rearVariantId) return items;

    // Customer-facing cart/checkout props only — no guide-source fluff.
    var baseProps = {
      'Vehicle': vehicle,
  ...
  function init(root) {
    ...
    function onStyleSelect(style) {
      ...
      loadDesigns(col).then(function (products) {
        renderDesigns(designsEl, products, function (btn, product) {
          var titleEl = btn.querySelector('span');
          state.selectedDesign = {
            variantId: (product && product.variantId) || btn.getAttribute('data-variant-id'),
            title: (product && product.title) || (titleEl ? titleEl.textContent : ''),
